...
// Make sure position is in object space and not in world space when using the procedural instancing feature
...
// Noise value lies between -1 and 1
float3 GetNoiseColor () {
	#if defined(UNITY_PROCEDURAL_INSTANCING_ENABLED)
		float noise = _Noise[unity_InstanceID];
        
        // Return grayscale color if positive, otherwise return red
